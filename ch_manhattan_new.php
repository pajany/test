...
$status_query = mysql_query("select * from Cronstatus where State='Manhattan' and Contenttype = 'CH' order by Id desc limit 1");
...
if($cron_count == '0' || $state_failed == 'Manhattan' || $state_failed == '' ) {
...
if(($contenttype=='CH')&&($state_failed == 'Manhattan')&&($aid_failed!=''))
		...
		if(isset($info['http_code'])&&($info['http_code']==200))
		{
			...
			$insert_query_value = '';
			   ...
			   $i;
				...
				echo $insert_query = "insert into CH_Manhattan (	house_number,Street,Complaint_Date,Complaint,SR,Apt,Complaint_Condition,Condtion_Detail,Location,zipcode) VALUES ".$insert_query_value;
				//echo "==>".$insert_query;
			...
			//}
				...
				$endhousenum       = $result_row['house_number'];
					...
					echo "update Cronstatus set Cron_status ='P',  State = '".$statename."',HouseNumber = '".$endhousenum."',Street = '".$endhousestreet."',Contenttype = 'CH',StateId = '".$endaid."',updated_time = '".date('Y-m-d H:i:s')."' where Id = '".$cronid."'" ;
