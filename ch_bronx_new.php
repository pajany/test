...
$status_query = mysql_query("select * from Cronstatus where State='Bronx' and Contenttype='CH' order by Id desc limit 1");
...
$today = date('Y-m-d');
$lastrowquery = "select Street,house_number,aid from Bronx order by aid desc limit 0,1";
...
$manhattan_query = "select aid,house_number,Street from Bronx $where_clause order by aid asc";
		...
		if(isset($info['http_code'])&&($info['http_code']==200))
		{
			...
			$insert_query_value = '';
			   ...
			   $i;
				...
				echo $insert_query = "insert into CH_Bronx (	house_number,Street,Complaint_Date,Complaint,SR,Apt,Complaint_Condition,Condtion_Detail,Location,zipcode) VALUES ".$insert_query_value;
				//echo "==>".$insert_query;
